...
def find_nearest_station(lat: float, lon: float, max_distance_km: int) -> dict | None:
    try:
        url = (
            f"https://airapi.airly.eu/v2/installations/nearest?lat={lat}&lng={lon}&maxDistanceKM={max_distance_km}"
              )
        ...
        if not stations:
            return None
        # Airly returns nearest installations first
        station = stations[0]
        return {
            "id": station["id"],
        ...
        print(f"Airly request failed: {e}")
    return None
# Example of the output {'id': 2464, 'latitude': 50.508196, 'longitude': 19.414377}


# x = find_nearest_station(50.5082, 19.4148, 5)
# print(x)

# lon = 
# lat = 
# 50.50910227348566, 19.412999254704324

def fetch_air_quality_data(lat: float, lon: float):
    url = f"https://airapi.airly.eu/v2/measurements/point?lat={lat}&lng={lon}"
    ...
    x = json.dumps(air_quality_data, indent = 2)
    return x

fetch = fetch_air_quality_data(57.77194936436369, 14.15443446524945)
print(fetch)
